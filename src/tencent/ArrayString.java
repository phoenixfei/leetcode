...
public class ArrayString {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = 0; j <= i; j++) {
                if (i - j == 0)
                    ...
                    dp[j][i] = true;
                else if (i - j >= 2 && s.charAt(i) == s.charAt(j) && dp[j + 1][i - 1])
                    dp[j][i] = true;
     ...
     * 字符变整数的两种方法，其一，利用Character；其二，'8'-'0'; 判断一个整数是否越过int边界，请在循环内部判断，注意变形。n >
     * (Integer.MAX_VALUE - d) / 10
     */
    public int myAtoi(String str) {
        ...
        if (firstChar == '-')
            ...
            dight = Character.getNumericValue(firstChar);
        else
            return 0;
        if (s.length() == 1)
            return (int) dight;
        ...
        if (flag == 1)
            return dight > Integer.MAX_VALUE ? Integer.MAX_VALUE : (int) dight;
