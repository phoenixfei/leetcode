...
/**
 * Collections 工具类常用方法: 1.排序 下述三个方法都可以；此时o1和o2是引用类型变量；对于基本数据类型int，直接使用减号即可。
 * 查看源码可以发现，compareTo与compare都是Integer方法，只不过compare还是静态方法 eturn o2.intValue() -
 * o1.intValue(); Integer.compare(o1.intValue(), o2.intValue());
 * o2.compareTo(o1); 2.查找,替换操作 3.同步控制(不推荐，需要线程安全的集合类型时请考虑使用 JUC 包下的并发集合)
 */
public class ArraysCollctions {
    /**
     * 常用方法： void reverse(List list)//反转 void shuffle(List list)//随机排序 void
     * sort(List list)//按自然排序的升序排序 void sort(List list, Comparator
     * c)//定制排序，由Comparator控制排序逻辑 void swap(List list, int i , int j)//交换两个索引位置的元素
    ...
    /**
     * 常用方法： int binarySearch(List list, Object key)//对List进行二分查找，返回索引，注意List必须是有序的
     * int max(Collection coll)//根据元素的自然顺序，返回最大的元素。 int max(Collection coll,
     * Comparator c)//根据定制排序，返回最大元素，排序规则由Comparatator类控制 int min(Collection coll)
     * int min(Collection coll, Comparator c) void fill(List list, Object
     * obj)//用指定的元素代替指定list中的所有元素。 int frequency(Collection c, Object o)//统计元素出现次数
     * int indexOfSubList(List list, List
     * target)//统计target在list中第一次出现的索引，找不到则返回-1，类比int lastIndexOfSubList(List
     * source, list target). boolean replaceAll(List list, Object oldVal, Object
     * newVal), 用新元素替换旧元素
     */
    ...
    /**
     * Arrays的常用方法 sort(),可以定制比较器 binarySearch() equals() fill() asList() toString()
     */
