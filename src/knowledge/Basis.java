...
public class Basis {
    ...
    /**
     * 交换两个数的三种方法使用总结 1.利用第三方变量 2.利用两数之和 3.利用异或：两个操作数的位中，相同则结果为0，不同则结果为1。 总结：
     * 方式一是最常见的，可读性高，但是需要在内存中存放临时变量，但是对于现在来说，需要的内存空间很小，而且存放临时变量的内存很快就会释放，不存在问题。
     * 方式二有缺陷，当两个数相加之后，可能其结果超出了变量类型能表达的最大范围，这个时候结果就会出问题，不推荐使用 方式三效率是最高的，但是可读性不是很好。
     * 在程序中我们应该尽可能的使用方式一，提高程序的可读性。但是需要了解方式三，以及方式三的原理。 强调：
